package com.algaworks.ecommerce.aula09;
...
//                " where p.preco = ANY (select precoProduto from Pedido where produto = p)";
    ...
    public void pesquisarComINExercicio() {
        String jpql = "select p from Pedido p where p.id in " +
                " (select p2.id from PedidoItem i2 join i2.pedido p2 join i2.produto pro2 join pro2.categorias c2 where c2.id = 2)";
	...
	public void pesquisarComExists() {
        String jpql = "select p from Produto p where exists (select 1 from PedidoItem ip2 join ip2.produto p2 where p2 = p)";
...
//         Bons clientes. Versão 2.
        String jpql = "select c from Cliente c where " +
                " (select sum(p.total) from Pedido p where p.cliente = c) > 500";
