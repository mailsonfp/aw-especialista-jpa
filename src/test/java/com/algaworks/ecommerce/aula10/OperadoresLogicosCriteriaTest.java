...
public class OperadoresLogicosCriteriaTest extends EntityManagerTest {
    ...
    public void usarOperadores() {
        ...
        criteriaQuery.where(
        		criteriaBuilder.or(criteriaBuilder.equal(root.get(Pedido_.status), StatusPedido.AGUARDANDO),criteriaBuilder.equal(root.get(Pedido_.status), StatusPedido.PAGO)),
    ...
    public void usarAnd() {
        ...
        // --- cada vírgula é um and
        criteriaQuery.where(criteriaBuilder.notEqual(root.get(Pedido_.total), new BigDecimal(499)),criteriaBuilder.equal(root.get(Pedido_.status), StatusPedido.PAGO));
